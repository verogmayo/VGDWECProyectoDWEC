...
function obtenerPalabrasRespaldo(nivel) {
  ...
  async function empezarJuego(nivel, arrayPalabras = null) {
    ...
    if (arrayPalabras === null) {
      ...
      console.log(`Obteniendo ${config.cantidad} palabras de ${config.longitud} letras en ${idioma === 'fr' ? 'francés' : 'español'}...`);
      arrayPalabras = await obtenerPalabrasAPI(config.cantidad, config.longitud, idioma);
